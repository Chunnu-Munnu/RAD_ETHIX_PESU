...
async def predict_chest_xray(file: UploadFile = File(...)):
    ...
    try:
        ...
        # FILTER findings: top 3 + any Critical Pneumonia (if not in top 3)
        ...
        logger.info(f"✅ Analysis complete: {len(result_findings)} findings detected")
        return response
    except Exception as e:
